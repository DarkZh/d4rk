      ...
      case 'info':
       ...
       case 'ytmp3':
           ytdl('mp3', value)
               .then(data => {
                   ...
                   let hasil = `鉁� Lagu Berhasil Di Download, silahkan klik link dan download hasilnya\nKlik link dibawah馃棥锔廫n\nJudul: ${judul}\n\nUkuran audio: ${size}\n\nLink: ${link}`
                   client.sendMessage(id, '[ESPERE UM POUCO...', MessageType.text)
       ...
       case 'ytmp4':
           ytdl('mp4', value)
               .then(data => {
                   const { judul, size, hasil: link } = data
                   let hasil = `鉁� Video Berhasil Di Download, silahkan klik link dan download hasilnya\nKlik link dibawah馃棥锔廫n\nJudul: ${judul}\n\nUkuran audio: ${size}\n\nLink: ${link}`
                   client.sendMessage(id, '[ESPERE UM POUCO...', MessageType.text)
                   client.sendMessage(id, hasil, MessageType.text)
               })
               .catch(err => {
       ...
       case 'twt':
           tweetdl(value)
               .then(data => {
                    ...
                    let hasil = `鉁� Berhasil! silahkan klik link di bawah untuk mendownload hasilnya!\nKlik link dibawah馃棥锔廫n\nSize: ${size}\n\nLink: ${link}`
                    client.sendMessage(id, '[ESPERE UM POUCO...', MessageType.text)
                    client.sendMessage(id, hasil ,MessageType.text)
               })
               .catch(err => {
       ...
       case 'tiktok':
           tiktokdl(value)
               .then(data => {
                    ...
                    let hasil = `鉁� Berhasil!!! Silahkan klik link dibawah ini untuk mendownload hasilnya! \nKlik link dibawah馃棥锔廫n\nJudul: ${deskripsi} \n\nDurasi: ${durasi}\n\nNama: ${nama}\n\nUrl: ${url}`;
                    client.sendMessage(id, hasil, MessageType.text)
               })
               .catch(err => {
                   console.log(err)
       ...
       case 'covid':
           corona()
               .then(data => {
                   ...
                   let hasil = `馃搶DATA WABAH COVID-19 TERBARU DI INDONESIA\n\n馃搷Positif ==> ${positif} \n馃搷Sembuh ==> ${sembuh} \n馃搷Meninggal ==> ${meninggal}`
                   client.sendMessage(id, hasil, MessageType.text)
               })
               .catch(err => {
                   console.log(err)
       ...
       case 'lirik':
           lirik(value)
               .then(data => {
                   ...
                   let hasil = `馃搷lirik lagu馃搷 *${value}* \n\n\n${lirik}`
                   client.sendMessage(id, hasil, MessageType.text)
               })
               .catch(err => {
                   console.log(err)
   ...
   case 'fb':
           fb(value)
               .then(data => {
                   ...
                   let hasil = `Pilih resolusi nya sayang馃槞 \n\n\n HD ${resultHD} \n\n\n SD ${resultSD}`
                   client.sendMessage(id, hasil, MessageType.text)
               })
               .catch(err => {
                   console.log(err)
       ...
       case 'igstalk':
           igStalk(value)
               .then(data => {
                   ...
                   client.sendMessage(id, '[WAIT] Stalking...鈴�', MessageType.text)
                   let hasil = `鉁˙iodata Instagram _${value}_ \n\n 馃Ф *Username* : ${Username}_ \n 馃寑 *Nama* : _${Name}_ \n 馃専 *Jumlah Followers* : _${Jumlah_Followers}_ \n 馃尃 *Jumlah_Following* : _${Jumlah_Following}_ \n 猸� *Jumlah_Post* : _${Jumlah_Post}_ `
                   client.sendMessage(id, hasil, MessageType.text)
               })
               .catch(err => {
...
case 'puisi2':
 ...
 case 'infogempa':
   ...
   case 'chord':
           chord(value)
               .then(data => {
                   const { result } = data
                   let hasil = `Nih chord lagu *${value}* sayang 鈾ワ笍\n\n  _${result}_ `
                   client.sendMessage(id, hasil, MessageType.text)
               })
               .catch(err => {
                   console.log(err)
 ...
 case 'wikien':
           wikien(value)
               .then(data => {
                   const { result } = data
                   let hasil = `*鈾伙笍According to Wikipedia馃椏:* \n\n  _${result}_ `
                   client.sendMessage(id, hasil, MessageType.text)
               })
               .catch(err => {
                   console.log(err)
  ...
  case 'cektanggal':
   ...
   case 'bitly':
           bitly(value)
               .then(data => {
                   const { result } = data
                   let hasil = `Nih kak udah jadi鈽ｏ笍 :) \n\n${result} `
                   client.sendMessage(id, hasil, MessageType.text)
               })
               .catch(err => {
                   console.log(err)
      ...
      case 'stickergif':
            if (isMedia) {
                if (mimetype === 'video/mp4' && message.duration < 10 || mimetype === 'image/gif' && message.duration < 10) {
                    ...
                    client.reply(from, '[WAIT] Sedang di proses鈴� silahkan tunggu 卤 1 min!', id)
